...
#include "cartographer/cloud/client/map_builder_stub_decentralized.h"
...
std::map<int, int> MapBuilderStub::LoadStateFromRemote(
        const std::string& remote_address, const bool load_frozen_state) {


    LOG(FATAL) << "Not implemented";

    proto::LoadStateFromFileRequest request;
    request.set_file_path(remote_address);
    request.set_client_id(client_id_);
    request.set_load_frozen_state(load_frozen_state);
    async_grpc::Client<handlers::LoadStateFromFileSignature> remoteclient(client_channel_);
    CHECK(remoteclient.Write(request));
    return FromProto(remoteclient.response().trajectory_remapping());
}
